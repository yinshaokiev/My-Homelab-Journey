...
* Check 1 (The Agent): Is the forwarder running?
```bash
sudo /opt/splunkforwarder/bin/splunk restart
```
* Check 2 (The File): Does the log file exist?
```bash
ls -l /var/log/auth.log (If missing: sudo systemctl start rsyslog)
```
* Check 3 (The Config): Is the input configured?
```bash
